...
struct MSG_JOINWATCH_REQ : public MSG_BASE_FORWARD
{
	MSG_JOINWATCH_REQ() {
		memset( this, 0, sizeof(MSG_JOINWATCH_REQ) );
		m_byCategory = Games_Protocol;
		m_byProtocol = JoinWatch_REQ;
	}
};

struct MSG_JOINWATCH_BRD : public MSG_BASE_FORWARD
{
	MSG_JOINWATCH_BRD() {
		memset( this, 0, sizeof(MSG_JOINWATCH_BRD) );
		m_byCategory = Games_Protocol;
		m_byProtocol = JoinWatch_BRD;
	}
};


struct MSG_QUITWATCH_REQ : public MSG_BASE_FORWARD
{
	MSG_QUITWATCH_REQ() {
		memset( this, 0, sizeof(MSG_QUITWATCH_REQ) );
		m_byCategory = Games_Protocol;
		m_byProtocol = QuitWatch_REQ;
	}
};

struct MSG_QUITWATCH_BRD : public MSG_BASE_FORWARD
{
	MSG_QUITWATCH_BRD() {
		memset( this, 0, sizeof(MSG_QUITWATCH_BRD) );
		m_byCategory = Games_Protocol;
		m_byProtocol = QuitWatch_REQ;
	}
};
